...
1. User sets up a training session called a `drill` and begins being served
   optionally filtered clues one at a time, which they can pass or attempt to
   answer.
1. Drills can be ended at any time by the user. Else, they will end when the
   clue pool is empty. Stats are persisted; users can view past drill results.
